...
void draw() {
  ...
  histogram = new float[maxiterations+1];
  ...
  for (int j = 0; j < height; j++) {
    for (int i = 0; i < width; i++) {
      ...
      if(iterationsNeeded[i][j] == maxiterations){
        ...
        hue = hue * 1000;
        hue = hue % 1000;
        color color1 = color(hue, 1000, 1000); 
        color color2 = color((hue) + 1, 1000, 1000);
        color actualColor = lerpColor(color1, color2, hue % 1);
        
        //need to develop a gradient framework and an ability to logarithmically scale the gradients 
